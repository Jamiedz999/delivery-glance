...
Run:
    ...
    python3 -m http.server 4173 --directory docs/planning/prototypes
...
Scope note: this file now carries a visible **Full product · Future Work included** ribbon. The selected Status Story layout remains the Portfolio Core direction, but its ETA/Running Late scenarios document [Future Work 13](../future-work/13-add-travel-time-eta.md), not Sprint 3 scope. Implementation behaviour comes from the Core flow logic prototype above.
...
Scope note: this file now carries a visible **Full product · Future Work included** ribbon. The focused workspace layout remains useful, but its Matching Round and exception flows document [Future Work 15](../future-work/15-add-delivery-exceptions-and-reassignment.md) and [Future Work 17](../future-work/17-add-ranked-multi-courier-matching-round.md). Portfolio Core uses Direct Assignment; implementation behaviour comes from the Core flow logic prototype above.
