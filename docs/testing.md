...
| The public demo greets a first visitor with an empty board | `demo/DemoSelfHealTest.showsTheFictionalDeliveriesFromStartupWithoutAnybodyCallingTheReset` | With the switch on and a schedule configured, the two fictional Deliveries are there before any test creates one or posts a reset, each `AWAITING_COURIER` with its first transition attributed to the Dispatcher — the self-heal acts as a Staff Account rather than writing unattributed rows. |
| The demo keeps whatever state a visitor left it in | `demo/DemoSelfHealTest.putsADrivenDemoBackToTheStateTheWalkthroughStartsFrom` | A demo driven off the walkthrough — a stray Delivery, the Courier On Duty and sharing a position, a Tracking Link handed out — is back to the two Deliveries, an Off Duty Courier with no position, and a dead link after one pass of the same method the cron calls. |
| A demo that heals itself is a demo anyone can wipe | `demo/DemoSelfHealTest.healingTheDemoDoesNotOpenTheResetToAnybodyNewer` | After a scheduled reset, `POST /api/demo/reset` still refuses a Courier and still refuses the Dispatcher without a CSRF header. The schedule is an internal bean call and weakens no route. |
| The self-heal is only ever reached by hand | `demo/DemoSelfHealTest.keepsHealingAfterwards` | The configured cron is registered against `restoreTheWalkthroughState` itself — exactly one scheduled task, and its expression is the one the deployment set. Sitting through a real tick is the only stronger proof and is not one a suite can afford. |
| A deployment acquires a data-wiping timer it never asked for | `demo/DemoResetTest.isNotResetOnATimerUnlessADeploymentAsksForOne`, `demo/DemoResetDisabledTest.buildsNeitherTheResetNorTheSchedulerThatWouldCallIt` | With the switch on and no schedule, no self-heal bean exists at all; with the switch off, neither the reset nor the self-heal is built, whatever the schedule says. |
